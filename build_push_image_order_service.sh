...
# and tag it with the name 'order-service' and version 'latest'.
docker build -t ghcr.io/deltarfd/order-service:latest .
...
# Push the Docker image ('order-service:latest') to the container registry ('ghcr.io').
docker push ghcr.io/deltarfd/order-service:latest